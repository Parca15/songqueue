...
class TestSuperAdmin:
    ...
    async def test_venue_login_has_role(self, client, super_headers):
        ...
        await client.post(
            "/api/v1/venues",
            json=_venue_payload("Role Bar", "role_admin"),
    ...
    async def test_create_venue_requires_super(self, client):
        ...
        assert (
            await client.post("/api/v1/venues", json=_venue_payload("X", "x"))
    ...
    async def test_super_crud_venue(self, client, super_headers):
        ...
        create = await client.post(
            "/api/v1/venues",
            json=_venue_payload("CRUD Bar", "crud_admin"),
            headers=super_headers,
        )
        ...
        patch = await client.patch(
            f"/api/v1/super/venues/{vid}",
            json={"name": "CRUD Renombrado"},
            headers=super_headers,
        )
        ...
        delete = await client.delete(
            f"/api/v1/super/venues/{vid}", headers=super_headers
        )
    ...
    async def test_super_manages_any_venue_queue(self, client, super_headers):
        ...
        create = await client.post(
            "/api/v1/venues",
            json=_venue_payload("Q Bar", "q_admin"),
            headers=super_headers,
        )
        vid = create.json()["id"]
        add = await client.post(
            f"/api/v1/queue/venue/{vid}/add",
            json={
                "youtube_id": "super_q_1",
                "device_fingerprint": "fp_super_q_01",
                "title": "SQ",
            },
        )
        ...
        move = await client.post(
            f"/api/v1/queue/venue/{vid}/move",
            json={"item_id": item_id, "new_position": 1},
            headers=super_headers,
        )
        assert move.status_code == 200
        play = await client.post(
            f"/api/v1/queue/venue/{vid}/play/{item_id}", headers=super_headers
        )
    ...
    async def test_venue_admin_cannot_use_super(self, client, super_headers):
        ...
        v1 = (
            await client.post(
                "/api/v1/venues",
                json=_venue_payload("Iso One", "iso_one"),
                headers=super_headers,
            )
        ).json()["id"]
        v2 = (
            await client.post(
                "/api/v1/venues",
                json=_venue_payload("Iso Two", "iso_two"),
                headers=super_headers,
            )
        ).json()["id"]
        login = await client.post(
            "/api/v1/auth/login", json={"username": "iso_one", "password": "pass123"}
        )
        H = {"Authorization": f"Bearer {login.json()['access_token']}"}

        assert (await client.get("/api/v1/super/venues", headers=H)).status_code == 403
        assert (
            await client.patch(f"/api/v1/venues/{v2}", json={"name": "Hack"}, headers=H)
        ).status_code == 403
        assert (
            await client.get(f"/api/v1/venues/{v2}/qr", headers=H)
        ).status_code == 403
        # Pero sí su propio local
        assert (
            await client.get(f"/api/v1/venues/{v1}/qr", headers=H)
        ).status_code == 200
    ...
    async def test_super_password_reset(self, client, super_headers):
        ...
        create = await client.post(
            "/api/v1/venues",
            json=_venue_payload("Reset Bar", "reset_admin"),
            headers=super_headers,
        )
        vid = create.json()["id"]
        patch = await client.patch(
            f"/api/v1/super/venues/{vid}",
            json={"admin_password": "nueva123"},
            headers=super_headers,
        )
        assert patch.status_code == 200
        ...
        old = await client.post(
            "/api/v1/auth/login",
            json={"username": "reset_admin", "password": "pass123"},
        )
        assert old.status_code == 401
        new = await client.post(
            "/api/v1/auth/login",
            json={"username": "reset_admin", "password": "nueva123"},
        )
